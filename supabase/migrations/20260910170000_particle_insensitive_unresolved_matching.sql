create or replace function public.normalize_person_name_core_signature(input_value text)
returns text
language sql
immutable
...
tokens as (
  ...
  where btrim(token) <> ''
    and btrim(token) not in ('de', 'la', 'le', 'les', 'du', 'des', 'd', 'da', 'del', 'della', 'van', 'von')
)
...
token_keys as (
  ...
  union all
  select book_id, token_pos, legacy_author_name, core_signature_token as match_key
  from author_tokens
...
token_keys as (
  ...
  where signature_token is not null
  union all
  select book_id, token_pos, legacy_artist_name, core_signature_token as match_key
  from artist_tokens
  where core_signature_token is not null
),
artist_keys as (
...
grant execute on function public.normalize_person_name_core_signature(text) to authenticated;
grant execute on function public.normalize_person_name_core_signature(text) to service_role;