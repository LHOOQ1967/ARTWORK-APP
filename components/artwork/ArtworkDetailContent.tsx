...
import type { ArtworkWithRelations, ArtworkProposal, ArtworkDocument } from '@/app/(protected)/types/artwork'
...
async function deleteArtwork() {
  ...
  if (!res.ok) {
    // ✅ lire le texte brut, pas du JSON forcé
...
setArtwork(prev => {
  ...
  const documents: ArtworkDocument[] =
    prev.documents
      ...
      .map((d, index) => ({
        ...d,
        artwork_id: prev.id,              // ✅ garanti
        position: d.position ?? index + 1 // ✅ number garanti
      }))
