...
export default function EditArtistCategoryPage() {
  ...
  const [item, setItem] = useState<{ description: string; definition: string | null } | null>(null); const [error, setError] = useState(''); const [saving, setSaving] = useState(false)
  ...
  return <main style={{ padding: '96px 20px 56px', minHeight: '100vh', background: '#f3f5f1' }}><section style={{ maxWidth: 760, margin: '0 auto', padding: 30, background: '#fff', border: '1px solid #d7dfda', borderRadius: 12, color: 'black' }}><header style={{ textAlign: 'center', marginBottom: 28, paddingBottom: 20, borderBottom: '1px solid #e4e9e6' }}><p style={{ margin: 0, color: '#557067', fontSize: 12, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase' }}>Referentials</p><h1 style={{ margin: '10px 0 0', color: '#143b2d', fontSize: 'clamp(2rem, 5vw, 3.25rem)' }}>Edit artist category</h1></header>{error && <p style={{ color: '#a22' }}>{error}</p>}<label style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 12, marginBottom: 12 }}>Description<input style={fieldStyle} value={item.description} onChange={e => setItem({ ...item, description: e.target.value })} /></label><label style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 12, marginBottom: 12 }}>Definition<textarea style={{ ...fieldStyle, minHeight: 110 }} rows={4} value={item.definition ?? ''} onChange={e => setItem({ ...item, definition: e.target.value })} /></label><div style={{ textAlign: 'right' }}><button className="edit-button" onClick={() => router.back()}>Cancel</button> <button className="edit-button" disabled={saving} onClick={save}>{saving ? 'Saving...' : 'Save'}</button></div></section></main>
}
