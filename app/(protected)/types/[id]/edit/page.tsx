...
export default function EditBookTypePage() {
  const { id } = useParams<{ id: string }>(); const router = useRouter(); const [item, setItem] = useState<{ type_number: string | null; description: string | null; full_name: string | null } | null>(null); const [error, setError] = useState(''); const [saving, setSaving] = useState(false)
  ...
  return <main style={{ padding: '96px 20px 56px', minHeight: '100vh', background: '#f3f5f1' }}><section style={{ maxWidth: 760, margin: '0 auto', padding: 30, background: '#fff', border: '1px solid #d7dfda', borderRadius: 12, color: 'black' }}><h1>Edit type of books</h1>{error && <p style={{ color: '#a22' }}>{error}</p>}<label>Type number<input style={fieldStyle} value={item.type_number ?? ''} onChange={e => setItem({ ...item, type_number: e.target.value })} /></label><label>Description<input style={fieldStyle} value={item.description ?? ''} onChange={e => setItem({ ...item, description: e.target.value })} /></label><label>Full name<input style={fieldStyle} value={item.full_name ?? ''} onChange={e => setItem({ ...item, full_name: e.target.value })} /></label><div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 20 }}><button className="edit-button" onClick={() => router.back()}>Cancel</button><button className="edit-button" onClick={remove}>Delete</button><button className="edit-button" disabled={saving} onClick={save}>{saving ? 'Saving...' : 'Save'}</button></div></section></main>
}
